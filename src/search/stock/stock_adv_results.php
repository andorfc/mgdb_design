 ...
 */
  ...
  if($gv1box == "true")
    $adv_results['query'] .= "
      LEFT OUTER JOIN mgdb.stock_genotypic_var sgv1 ON sgv1.id=s.id 
      LEFT OUTER JOIN mgdb.variation v1 ON v1.id=sgv1.variation";
      
  if($gv2box == "true")
    $adv_results['query'] .= "
      LEFT OUTER JOIN mgdb.stock_genotypic_var sgv2 ON sgv2.id=s.id 
      LEFT OUTER JOIN mgdb.variation v2 ON v2.id=sgv2.variation";
      
  if($gv3box == "true")
    $adv_results['query'] .= "
      LEFT OUTER JOIN mgdb.stock_genotypic_var sgv3 ON sgv3.id=s.id 
      LEFT OUTER JOIN mgdb.variation v3 ON v3.id=sgv3.variation";
 ...
 $query = "
   ...
   ) as sub1 LIMIT " . (int) $search_limit;
...
function getParent($parent, $DBConn, $adv_results) {
  if ($parent > 0) {
    ...
    $adv_results['query'] .= " AND scp.stock1 = " . (int) $parent;
  }
  else {
...
function getPhenoVar($phenotype, $attribution, $DBConn, $adv_results) {
  ...
  if (strlen($attribution) > 0) {
    $adv_results['query'] .= " AND sp.name LIKE " . $DBConn->quote($attribution);
...
function getGV1($genvar1, $DBConn, $adv_results) {
  $adv_results['query'] .= " AND v1.name LIKE " . $DBConn->quote($genvar1 . '%');
...
function getGV2($genvar2, $DBConn, $adv_results) {
  $adv_results['query'] .= " AND v2.name LIKE " . $DBConn->quote($genvar2 . '%');
  $adv_results['criteria'] .= "You want only stocks with the 
...
function getGV3($genvar3, $DBConn, $adv_results) {
  $adv_results['query'] .= " AND v3.name LIKE " . $DBConn->quote($genvar3 . '%');
  $adv_results['criteria'] .= "You want only stocks with the 
...
function getName($name, $DBConn, $adv_results) {
  if (substr($name,-1) == " ") {
     $adv_results['query'] .= " AND LOWER(s.name) LIKE " . $DBConn->quote(trim(strtolower($name)));
  }
  else {
    $adv_results['query'] .= " AND LOWER(s.name) LIKE " . $DBConn->quote('%' . strtolower($name) . '%');
  }
...
function read_stock_syn($DBConn, $id) {
  $query_synonyms = "SELECT description FROM mgdb.description WHERE ID = " . (int) $id;
