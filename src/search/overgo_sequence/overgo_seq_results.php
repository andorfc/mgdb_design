 ...
 */
  ...
  /* Missing Rownum
  if(($arrCount > 250) && ($start != 0))
    $overgo_search = "SELECT ID, NAME, MEMO FROM (SELECT ID, NAME, MEMO, ROWNUM N FROM (" . $overgo_search . ")) WHERE N BETWEEN " . $start . " AND " . $finish; */

   //Grab the comments for each result and add them to our array
  /*for($i=0; $i<$arrCount; $i++)
  {
    $arrOvergo[$i]['comment'] = read_comment($DBConn, $arrOvergo[$i]['id']);
	$arrOvergo[$i]['synonym'] = str_replace("<br>", ", ", read_synonyms($DBConn, $arrOvergo[$i]['id']));
  } */
  ...
  if ($arrCount == 1 && $main != "initial_true") {
