...
/* file: ecr_data.php
 *
 * ecr: display the various sections of a ecr record; called via Ajax
 *
 * history:
 ...
 */
  ...
  logMessage("ecr_data.php: id=$id, type=$type");
  
  if (!$id) {
    reportError("No id given to ecr_data.php.");
    exit;
  }
  if (!$type) {
    reportError("No section type given to ecr_data.php.");
    exit;
  }

  $bauplan = $bauplan = new Bauplan('');
  $tmpl = $bauplan->template()->load('../templates/data_center/ecr_sections.bau');
  ...
  function show_overview($tmpl, $id, $DBConn) {
    ...
    if ($enzyme = read_enzyme($DBConn, $arrRecord['enzyme']))
    {
      $tmpl->get('enzyme_id')->replace($enzyme['id']);
      $tmpl->get('enzyme_name')->replace($enzyme['name']);
      $tmpl->get('enzyme_sec')->unmute();
    }
    
    if ($species = read_species($DBConn, $arrRecord['species']))
    {
      $tmpl->get('species')->replace($species['species']);
      $tmpl->get('species_sec')->unmute();
    }
    
    if (strlen($arrRecord["substrate_spec"]) > 0) {
      $tmpl->get('substrate')->replace($arrRecord["substrate_spec"]);
      $tmpl->get('substrate_sec')->unmute();
    }
    
    if ($propertys = read_properties($DBConn, $id))
    {
      $tmpl->get('properties')->loop($propertys);
    ...
    if (strlen($arrRecord["substrate_spec"]) <= 0 && strlen($comments) <= 0 &&
       !$pathways && !$enzyme && !$species && !$references && !$propertys) 
  ...
  function showAnnotations($tmpl, $id, $DBConn) {
    ...
    if (!$arrAnnotations || count($arrAnnotation) == 0) {
      $annotations = '<b>&nbsp;&nbsp;No annotations found for this Enzyme Catalyzed Reaction record.</b>';
    }
    else {
	...
	*/
	  ...
	  // Get the record
	  $query_record = "SELECT * FROM ENZ_CAT_REACTION WHERE ID = " . (int) $id;
   ...
   ****************************************************/
   ...
   function read_enzyme($DBConn, $enzyme)
   {
    ...
    {
      $query = "
       SELECT NAME, ID FROM GENE_PRODUCT WHERE ID = " . $enzyme;
   ...
   function read_species($DBConn, $species)
   {
    if(strlen($species) > 0)
    {
      $query = "
       SELECT SPECIES FROM SPECIES WHERE ID = " . $species;
      $stmt = make_query($DBConn,$query);
      $row = retrieve_row($stmt);
      return $row;
    }
   ...
   function read_pathways($DBConn, $id)
   {
      $query = "
       select
         a.ID, a.NAME
       from 
         META_PATH a, ID_NUM b, META_PATH_STEPS C
       where
         C.ID = b.ID and
         C.ID = a.ID
         and b.CURATION_LVL = 0
         and c.STEP_ENZYME = $id
       order by
         LOWER(a.NAME)";
      $stmt = make_query($DBConn,$query);
      $rows = get_all_rows($stmt);
