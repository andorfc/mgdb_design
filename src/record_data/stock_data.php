 ...
 */
  ...
  $query_record = "
    ...
    WHERE s.id = " . (int) $id . " AND idn.curation_lvl IN (0, 101)";
...
function showAnnotations($tmpl, $id, $DBConn) {
  ...
  $query_record = "SELECT * FROM stock WHERE id = " . (int) $id;
...
function showAssembly($id, $stock_name, $tmpl, $DBConn) {
  $sql = "
    ...
    WHERE stock_id=" . $DBConn->quote($id) . " OR stock_name=" . $DBConn->quote($stock_name);
  $sth = make_query($DBConn, $sql);
  $version = 0.0;
...
function show_available_from($id, $arrRecord, $tmpl, $DBConn) {
  ...
  if ($row = retrieve_row($sth)) {
    ...
    if ($row['id'] == 25725) {
      ...
      $coop_sql = "SELECT DISTINCT description FROM description WHERE id=" . (int) $id;
...
function show_description($id, $tmpl, $DBConn) {
  ...
  if (isset($arrDescription["description"])) {
    $tmpl->get('description')->replace(mgdb_safe_html(trim($arrDescription['description'])));
  ...
  while ((isset($arrImages[$img_count -1]['name']))
    ...
    $arrTblImages .= "
      <tr style='background-color: $bgcolor' valign='top' width='100%'>
        ...
        <td valign='top' width='60%'>" . mgdb_safe_html($arrImages[$img_count -1]['caption']) . "<br><br></td>
      </tr>";
    $img_count++;
