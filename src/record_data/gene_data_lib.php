...
function getAssembly($gm_set, $DBConn) {
  ...
  $sql = "
    ...
    WHERE ann.name = " . $DBConn->quote($gm_set);
...
//logMessage("Get gene model for $id, version $gm_version");
  if (!$gm_version) {
    ...
    $sql = "
      ...
      WHERE gene_name=" . $DBConn->quote($id) . " ORDER BY version DESC";
    $sth = make_query($DBConn, $sql);
    if ($row=retrieve_row($sth)) {
...
/* za_gene_models archived
      ...
      $sql = "SELECT version FROM za_gene_models WHERE gene_id=" . $DBConn->quote($id) . " ORDER BY version DESC";
      $sth = make_query($DBConn, $sql);
      if ($row=retrieve_row($sth)) {
...
/* za_gene_models archived
  if ($gm_version && $gm_version == $system['gm_set_v2']) {
    $sql = "
      ...
      WHERE gene_id = " . $DBConn->quote($id) . " AND version = 'RefGen_v2' 
    ...
    $sql = "
      ...
      FROM za_gene_models gm
      WHERE gene_id = " . $DBConn->quote($id) . " AND version = 'RefGen_v1'";
    $sth = make_query($DBConn, $sql);
    $arrRecord = retrieve_row($sth);
...
*/
    $sql = "
      ...
      WHERE gene_name=" . $DBConn->quote($id);
//logMessage("\n$sql\n");
    if ($gm_version) {
      $sql .= " AND version=" . $DBConn->quote($gm_version);
    }
    else {
...
function getGenomeMetadata($assembly_name, $DBConn) {
  $sql = "SELECT * FROM chado.genome_metadata WHERE assembly_name=" . $DBConn->quote($assembly_name);
  $sth = make_query($DBConn, $sql);
  if ($row=retrieve_row($sth)) {
    return $row;
  }
...
//  else {
    $sql = "
      ...
      FROM chado.transcript
      WHERE feature_id=" . $arrRecord['feature_id'] . " AND version=" . $DBConn->quote($version);
    $sth = make_query($DBConn, $sql);
    $rows = get_all_rows($sth);
//  }//not v1 or v2
