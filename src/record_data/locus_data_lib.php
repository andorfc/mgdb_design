...
*/
    ...
    $sql = "
      ...
      WHERE b.in1 = '1232902' AND a.id =" . (int) $id;
...
return;
  ...
  function showLinkageGroup($tmpl, $arrRecord, $locus_id, $DBConn) {
    if (strlen($arrRecord['linkage_group']) > 0) {
      ...
      $html = "";
      $query_length = "SELECT length, units FROM locus_length WHERE id = " . (int) $locus_id;
  ...
  function loadNeighbors($mapset, $tmpl, $id, $loc, $centimorgan, $refreshed, $DBConn) {
      ...
      switch ($mapset) {
        ...
        case "Genetic":
          ...
          break;
        default:
          $map_select .= "like " . $DBConn->quote($mapset . ' %');
          break;
       }
      $sql = "
        SELECT map, value
      ...
      if (!($map_row = retrieve_row($sth))) {
        ...
        $sql = "
            ...
            WHERE l.id = lc.id AND l.id = i.id
                  ...
                  AND lc.value > " . ($map_row['value'] - 0.01 - (float) $centimorgan) . "
                  AND lc.value < " . ($map_row['value'] + 0.01 + (float) $centimorgan) . "
            ORDER BY lc.value";
        $sth = make_query($DBConn, $sql);
        $count = 0;
...
*/
 ...
 function show_img_table($tmpl, $id, $DBConn) {
              ...
              && (strlen($arrImages[$img_count-1]['name']) > 0))
    {
      ...
      $img_results[$img_count - 1]['caption'] = mgdb_safe_html($arrImages[$img_count -1]['caption']);
  ...
  function get_sequence($locusname, $DBConn) {
    if(substr(strtolower($locusname),0,2) == "mu") {
      $query_seq = "
          ...
          from z_sequence a
          where LOWER(a.SEQ_TITLE) LIKE " . $DBConn->quote('%' . strtolower($locusname) . '%');
