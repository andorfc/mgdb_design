(function () {
  ...
  /* ── Records by prefix figure ───────────────────────────────────────────── */

  /* The census is rendered server side into #bac-chart-data by the same GROUP
     BY that fills the metric cards, so this draws without a request of its
     own. */
  function initFigure() {
    var data = readJson('bac-chart-data');
    if (!data || !data.bars || !data.bars.length) { return; }

    var bars = data.bars;

    var table = byId('bac-prefix-table');
    if (table) {
    ...
    var el = byId('bac-prefix-chart');
    if (!el) { return; }
    var height = Math.max(320, ordered.length * 40 + 110);
    el.style.height = height + 'px';

    /* Margins are sized from the figure, not fixed. Prefix group names run long --
       "c-prefix (EcoRI, MboI)" is 22 characters -- so the desktop gutter is
