<?PHP
/* file: bp_functions.php
 *
 * purpose: helper functions for displaying a body part record.
 *
 * history:
...
function check_id($id, $DBConn) {
  ...
  $query_verify = "
   SELECT A.ID 
   FROM TERM A, ID_NUM B 
   WHERE A.TYPE = 32466 AND A.ID = B.ID AND A.ID = " . $iid . " AND B.CURATION_LVL = 0";
  $stmt_verify = make_query($DBConn,$query_verify);
  $arrVerify = retrieve_row($stmt_verify);
  if($arrVerify["ID"] > 0)
  {
    $query_record = "SELECT NAME FROM TERM WHERE ID = " . $iid;
