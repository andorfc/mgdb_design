<?PHP
/* file: trait_analysis_functions.php
 *
 * purpose: helper functions for displaying a trait analysis record.
 *
 * history:
...
function check_id($id, $DBConn) {
  ...
  if($arrVerify['id'] > 0)
  {
    $query_record = "SELECT NAME FROM TRAIT_ANALYSIS WHERE ID = " . $iid;
...
function get_nav_array() {
  ...
  return array(
    ...
    array('nav_name' => 'Other Analyses',
          'nav_id0' => 'other_analyses',
          'is_checked' => 'checked'
    ),
  );
...
function get_section_array() {
  ...
  return array(
    ...
    array('color1' => 'lite_grey',
          'section_name' => 'Other Analyses',
          'dom_id1' => 'other_analyses',
          'dom_var' => 'other_analyses_cal'
    ),
  );
}//get_section_array
