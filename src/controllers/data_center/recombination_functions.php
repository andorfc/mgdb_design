<?PHP
/* file: recombination_functions.php
 *
 * purpose: helper functions for displaying a recombination record.
 *
 * history:
...
function check_id($id, $DBConn) {
  ...
  $query_verify = "
    SELECT A.ID FROM RECOMB A, ID_NUM B 
  ...
  if (isset($arrVerify['id'])) {
    $query_record = "SELECT * FROM RECOMB WHERE ID = " . $iid;
