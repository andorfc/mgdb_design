...
function getIssues($acc) {
  ...
  $jql = "project=ASMBLY AND text~'$acc'";
...
function getMapID($map_name, $DBConn) {
  $sql = "SELECT id FROM map WHERE name='$map_name'";
...
function showDescription($tmpl, $id, $DBConn) {
  ...
  // Get bin number
  $query_bin = "SELECT * FROM probe_bin WHERE ID=$id";
...
function showTop($tmpl, $id, $DBConn) {
  ...
  // Get synonyms
  $synonyms = array();
  $query = "SELECT synonyms FROM synonyms WHERE id=$id";
  $stmt = make_query($DBConn, $query);
  if ($stmt) {
    while ($row = retrieve_row($stmt)) {
