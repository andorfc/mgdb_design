<?php
  ...
  include_once('./controllers/data_center/stock_functions.php'); // for building filter UI
  include_once('./include/breeders_functions.php');
  include_once('./include/stock_adv_functions.php'); // for filter functions
...
///////////////////////////
  ...
  function buildFilterUI($dbh, $template, $vars) {
    ...
    for ($i=0; $i<count($states); $i++) {
        $states[$i]['selected'] = "";
        if (isset($vars['states']) && in_array($states[$i]['state'], $vars['states'])) {
    ...
    for ($i=0; $i<count($countries); $i++) {
        ...
        if (isset($vars['countries']) && in_array($countries[$i]['country'], $vars['countries'])) {
    ...
    for ($i=0; $i<count($developers); $i++) {
        ...
        if (isset($vars['developers']) && in_array($developers[$i]['id'], $vars['developers'])) {
    ...
    for ($i=0; $i<count($sources); $i++) {
        $sources[$i]['selected'] = "";
        if (isset($vars['sources']) && in_array($sources[$i]['id'], $vars['sources'])) {
    ...
    if (isset($vars['filter-state']) && $vars['filter-state'] == "on") {
      $template->get('state-checked')->unmute();
    }
    
    if (isset($vars['filter-developer']) && $vars['filter-developer'] == "on") {
      $template->get('developer-checked')->unmute();
    }
    
    if (isset($vars['filter-source']) && $vars['filter-source'] == "on") {
      $template->get('source-checked')->unmute();
    }
    if (isset($vars['filter-country']) && $vars['filter-country'] == "on") {
 ...
 //  echo "<pre>";var_dump($vars);echo "</pre>";
    ...
    if (isset($vars['states']) && count($vars['states']) > 0) {
    ...
    if (isset($vars['developers']) && count($vars['developers']) > 0) {
    ...
    if (isset($vars['sources']) && count($vars['sources']) > 0) {
    ...
    if (isset($vars['countries']) && count($vars['countries']) > 0) {
