...
function showEditForm($mgdb) {
  ...
  // Load edit form
  $tmpl = $mgdb->get('body')->load('templates/curation/genome-issue-form.bau');
...
logVarDump($_FILES, "Files uploaded:\n");
  ...
  $tmpl = $mgdb->get('body')->load('templates/curation/genome-issue-submitted.bau');
  
//  $tmpl->get('issue_key')->replace($issue_key);
//  $tmpl->get('organism')->replace($organism);
//  $tmpl->get('genotype')->replace($genotype);
  $tmpl->get('genome_assembly')->replace($genome_assembly);
  $tmpl->get('gene_model')->replace($gene_model);
  $tmpl->get('gene_model_version')->replace($gene_model_version);
  $tmpl->get('chromosome')->replace($chromosome);
  $tmpl->get('chr_start')->replace($chr_start);
  $tmpl->get('chr_end')->replace($chr_end);
  $tmpl->get('first_acc')->replace($first_acc);
  $tmpl->get('last_acc')->replace($last_acc);
  $tmpl->get('sub_name')->replace($sub_name);
  $tmpl->get('email')->replace($email);
  $tmpl->get('affiliation')->replace($affiliation);
  $tmpl->get('position')->replace($position);
  $tmpl->get('description')->replace($description);
  ...
  if (isset($_FILES['attachment']) && $upload_error == '') {
    $tmpl->get('attachment-name')->replace($_FILES['attachment']['name']);
