...
<!-- BEGIN GENERATED INDEX — tools/admin_index.py -->

## What is outstanding

85 entries have been raised. **73 are open** and **12 are closed** — applied, implemented, or recorded as needing no administrator at all. Regenerated by `tools/admin_index.py` on 2026-09-25; the entry is always the authority, this table is only a way in.

Grouped by who has to act, because that is how the list gets worked. An entry whose fix is already written and is waiting only on a privilege says so in its own **Status**.
...
| **AD-065** | A `phpinfo()` dump is published, unauthenticated, on the production site |
| **AD-070** | The sequence service refuses MaizeGDB's own servers, so two sequence downloads are dead on production |
| **AD-081** | The search suggestion indexes and the dashboard cache must be rebuilt after every database reload; a nightly job does it |
| **AD-083** | `temp/bundles/` must be writable by httpd on each production host, or bundled pages serve their files one by one |
...
| **AD-084** | `fusarium.maizegdb.org` puts off-campus visitors through a Cloudflare bot check, so no browser elsewhere could load its models |
...
| **AD-063** | Two phenotype term references the curation cannot reach |
...
| **AD-069** | Eleven news items in data/news.xml share an id with another item |
| **AD-072** | Rows pointing at records that do not exist, found by the API's own count checks |
| **AD-073** | 68 PubMed IDs and 4 DOIs each sit on two different reference records |
| **AD-074** | Literature curation falls to a tenth of its 1990s rate between 2005 and 2019, and two collections stop entirely |
| **AD-085** | Annotation files on download.maizegdb.org carry format defects that every reader has to work around |
...
| **AD-064** | Nested quotes silently strip `-num_threads` and the custom column list from every table/text BLAST job |
| **AD-066** | The modern curator login needs a per-instance token secret, and its hardening leaves two DBA-side residuals |
| **AD-067** | SNPversity's VCF and HapMap exports write to a hostname that does not resolve |
| **AD-068** | Apache gives up on php-fpm after 60 seconds, and SNPversity queries take minutes |
| **AD-080** | The Foldseek analysis exists only inside `foldseek.maizegdb.org`, and its viewer misreports some TM-scores |
| **AD-082** | `fusarium.maizegdb.org` refuses four species' ESMFold models, and `/fusarium` needs a decision about its host |

### No administrator — 8 open
...
| **AD-071** | `mgdb.maize_history` holds the 1994 MaizeDB event twice |
...
| | Outcome |
| --- | --- |
| AD-009 | the feedback half is resolved in application code, 2026-09-01; the gene-model report is… — Feedback and issue-report collectors exist only in the legacy shell |
| AD-017 | resolved, no administrator action needed — The 2025 UniformMu resource paper has no link on file (resolved) |
| AD-019 | not an administrator request — `data/protein_structure/` is built on the server and is not in the deploy manifest |
| AD-062 | not an administrator request — `all_text_search` mixes identity text with commentary, and the source-to-type map is not recorded anywhere |
| AD-026 | implemented (2026-08-28), with one table outstanding — 289 tables lost their SELECT grant to the application role, and `chado.genome_information` no longer exists |
| AD-048 | implemented (2026-09-03) — BLAST could not write its query file on the development instance |
| AD-050 | implemented (2026-09-03) — BLAST jobs ran but every result file came back empty |
| AD-056 | applied — the move and the redirects are live and verified; the final deletion is the… — Retired MGC committee sites are parked on the dev server, not deleted |
| AD-057 | applied and verified — five 301s, five pages still served, site map regenerated — Five short routes retired to 301s; their pages are still served |
| AD-058 | applied and verified — three 301s, the Map hub card removed, site map regenerated (129… — The three B73 RefGen_v2 locus tools are retired |
| AD-059 | applied and verified — Orphaned tool backends parked, and every remaining link to a retired route removed |
| AD-060 | applied and verified — eleven 301s, site map regenerated, `/unsubscribe` retired to… — Ten old or broken pages retired; /unsubscribe deliberately kept |

<!-- END GENERATED INDEX -->

> **Three numbers were issued twice** and were separated on 2026-09-06, so that an AD number names one thing. The later entry of each pair moved: the `all_text_search` source-to-type map from AD-021 to **AD-062**, the unreachable phenotype term references from AD-036 to **AD-063**, and the BLAST nested-quote argument loss from AD-054 to **AD-064**. Every reference in the code and in the README moved with them. AD-001 to AD-064 is now unique and has no gaps.
---
## AD-001 — Add a DOCTYPE to the global page shell
...
- **Proposed change:** `CREATE INDEX CONCURRENTLY` on `chado.gene_model (locus_name)`, `chado.transcript (transcript_name)`, and `perm_tables.id_ontology (id, table_name)`. **`all_gene_model_data` no longer needs one:** the only page that read it was the gene hub's "Download all data for a list of gene models", and on 2026-09-11 that was rewritten to read `chado.gene_model` and `perm_tables.id_ontology` directly, both of which are index-served on the columns it uses. Three gene models went from minutes to 0.2 s and two thousand to 0.5 s. Whether anything on the legacy side still reads the view is worth one grep before it is dropped.
...
## AD-062 — `all_text_search` mixes identity text with commentary, and the source-to-type map is not recorded anywhere
...
**Corrected 2026-09-14. The column is still empty for 99% of the literature,
but the premise below — that there is therefore nothing to render — was
wrong.** A second store holds DOIs for 8,493 references, and the reference hub
now reads it. What remains is a smaller backfill, and a code change on the
record pages. The original entry follows, with the corrections marked.
...
- **Measured on the development instance:** 538 of 55,171 references carry a
  ...
  them.
- **Correction — the DOI lives in two places.** `mgdb.ext_db_key` under
  `db_person = 2738676`, "Digital Object Identifier (DOI), -", carries a DOI
  for **8,493 of 54,900 curated references**, and only 489 of those are also in
  `mgdb.reference.doi`. Real coverage is **8,479 references, 15.5%**, once the
  free-text junk in both stores is discarded — not 1.0%. The same table also
  holds a **PubMed ID for 8,949 references** under `db_person = 134209`, so
  there *is* an identifier to resolve a backfill against; the entry's claim
  that there is not refers only to a column on `mgdb.reference`.
- **Why the application cannot fix it:** ~~There is nothing to render.~~ For
  8,493 references there is: reading both stores is a code change, and
  `/data_center/reference` made it on 2026-09-14
  (`referenceDoiSql()` in `search/reference/reference_search_lib.php`). For the
  remaining ~46,400 the entry stands — nothing in the database holds a DOI for
  them, and the citation strings do not carry one either.
- **What is needed:** A backfill for the references that have no DOI in either
  store, most cheaply by matching the **8,949 PubMed IDs** against PubMed or
  Crossref first and title-and-year for the rest, and then a curation step that
  populates it going forward. Settling on one of the two stores as the place a
  DOI belongs would be worth doing at the same time.
...
`/legacy_curation/curationIndex_test.php` answers **HTTP 200 on
www.maizegdb.org, maizegdb.org and the development instance**. Its second line,
before any authentication check, is `phpinfo();`. The response is the phpinfo
page: ~76 KB on production, ~84 KB on dev.

What that publishes, to anyone, with no session:

- the host's operating system, hostname and kernel build
- the PHP version, its `Server API`, and the paths of its configuration files
- `DOCUMENT_ROOT` and the rest of the server environment
- the full loaded-extension list, including `pdo_pgsql`

A `phpinfo()` page is the standard first step in fingerprinting a host: it names
the software to look up advisories for and the paths to aim at.

**The file is a login form.** After `phpinfo()` it builds an authentication
query by string concatenation —

```php
$userName = $_POST["username"];
$loginQuery = "select ID, FIRST_NAME, LAST_NAME, PASSWORD, CURATION_LVL, EMAIL
               from ANNOTATION_AUTHOR where trim(USERNAME) = '" . trim($userName) . "'";
```

— and compares `$password` to the stored value with `!=`, so passwords are held
in that table in a form that can be compared directly. **That query is not
reachable today.** The file's `require("../html/include/db-api.php")` resolves
to a path that does not exist, so execution fatals immediately after the
phpinfo output. The disclosure is live; the injection behind it is one corrected
`require` away from being live, and nothing in the file records that.

**It is not one file.** `/legacy_curation/` is 654 files and 9.4 MB of pre-2012
CGI in the web root, of which **165 build SQL by concatenating a variable into a
`WHERE` clause**. The directory answers 200 with an index page of its own
("Sanford's Document Index"). The 2026-09-05 injection audit deliberately left
this tree alone, and was right to: it is a scope decision, not a file-by-file
patch. This entry is that decision being asked for. `tools/car/examples/`, 44
files of example code that executes in the web root, is the same question at a
much smaller scale — its injections were fixed, but it should probably not be
published at all.

**What is being asked for** is a decision and then a deletion, not a patch. The
tree serves no route the redesign uses, nothing links to it, and it is not in
`deploy/manifest.txt`. Patching 165 files of dead pre-2012 CGI would be work
spent making unreachable code safe.

If the decision has to wait, the disclosure should not: deleting
`curationIndex_test.php`, or denying `/legacy_curation/` in the web server
configuration, closes the published phpinfo immediately and independently.

- **Required administrator:** whoever owns the production web root (deletion, or
  a `Require all denied` on the directory)
- **Status:** proposed. **Verified live on production 2026-09-06**; the
  fingerprinting disclosure is real today, the SQL injection behind it is not
  currently reachable.
...
The `/login` page was rebuilt on the design system on 2026-09-06, and the login
mechanism was hardened at the same time. The code changes are done and deployed
on the development instance; three things touch other hosts or the database and
so are recorded here.

**1. `auth_secret` in `conf/mgdb.conf` — needed on every other instance.**
Curator sessions are now carried by a signed session token (HMAC-SHA256) in
place of the old plaintext-password cookie. The signing key is read from an
`auth_secret=` line in `conf/mgdb.conf` — server-only, not in the repository and
not in `deploy/manifest.txt`, exactly like the DB credentials beside it. It was
generated and added on the development instance
(`openssl rand -hex 32`). **Production, and any other instance the login ships
to, needs its own `auth_secret` line** (a fresh 32-byte hex value; it must not be
shared between instances). If the line is absent the code falls back to deriving
a key from `DB_PASS` so logins still work, but the explicit value is preferred
and is what lets tokens stay valid across an unrelated DB-password rotation.

- **Required administrator:** whoever owns `conf/mgdb.conf` on each instance.
- **Status:** proposed for production and any other target; already added on the
  development instance.

**2. Existing curators are logged out once.** The old cookies held the raw
password; the new mechanism rejects them, so every currently-logged-in curator
is prompted to log in again a single time after this ships. This is expected and
desirable — it flushes the plaintext passwords out of browsers — and needs no
action, but the team should know it will happen.

**3. The plaintext `annotation_author.password` column is now unused by the
redesigned paths, but still exists.** The modern login verifies against the
bcrypt `mgdb_pw_hash` only, and the site's record pages no longer compare the
plaintext column (their annotation-ownership check is now id + username; see the
nine `record_data/*_data.php` files changed in this work). The column is still
written by `controllers/static/create_account.php` and still read by the
pre-2012 `legacy_curation/` CGIs — the same tree AD-065 covers. Dropping the
column is a DBA action that should wait until those legacy CGIs are retired.

- **Required administrator:** database owner (column removal), gated on the
  `legacy_curation/` decision in AD-065.
- **Status:** proposed; deferred behind AD-065.

---

## AD-067 — SNPversity's VCF and HapMap exports write to a hostname that does not resolve

Two of SNPversity 1.0's three output formats produce a link that cannot be
followed by anyone outside the tool's own network, and the file behind it is
not published either. Measured against the live engine on 2026-09-06, with real
queries, creating nothing on this side:

```
POST send.php … outputFormat=vcf
  → <a href=http://david1.usda.iastate.edu/Diversity/tassel/output/O<query>.vcf>View File</a>

POST send.php … outputFormat=hapmap
  → <a href=http://david1.usda.iastate.edu/Diversity/tassel/output/O<query>.hmp.txt>View File</a>
```

- **`david1.usda.iastate.edu` has no DNS record at all.** `getent hosts` returns
  nothing and a connection attempt fails before any HTTP request is made. It is
  presumably the engine host's old internal name.
- **Rewriting the host onto the public one does not recover the file.**
  `http://snpversity.maizegdb.org/Diversity/tassel/output/O<query>.vcf` returns
  **404**, as do `1_O<query>.vcf`, `O<query>.hmp.txt` and `O<query>.txt`. The
  `.json` pages of the *same query id* are served from that same directory and
  return 200, so this is not a directory permission: the export file is not
  written where the public server can see it, or is not written at all.

The `browser` output format — the interactive grid, and the only one anyone can
see the results of — is unaffected and works normally.

**How the modern page handles it in the meantime.** `/snpversity` still offers
all three formats. The API rewrites the returned link onto the public host,
issues a `HEAD` against it, and only presents a download when that returns 200;
otherwise the reader is told the file was not produced and pointed at the two
alternatives that do work — the grid, which downloads as TSV or CSV covering
every page of the result, and SNPversity 2.1 for a VCF over B73 v5. The form
also says so *before* the query is run, so nobody waits several minutes for a
dead link. If the export is fixed on the engine, the page starts offering the
download with no change here.

- **Required administrator:** whoever maintains snpversity.maizegdb.org — the
  export path TASSEL writes to, and the hostname written into the link
- **Status:** proposed. Worked around client-side; the workaround is honest
  rather than a fix, and reverses itself automatically if the engine is repaired.

---

## AD-068 — Apache gives up on php-fpm after 60 seconds, and SNPversity queries take minutes

Measured on the development instance 2026-09-06 with a deliberate 75-second
sleep through the full stack:

```
GET /zz_sleep_probe.php?n=75   →  504 Gateway Timeout at 60.05 s
```

The probe was created, measured once, and deleted; its absence was verified.

`/etc/httpd/conf.d/php.conf` routes PHP through
`SetHandler "proxy:unix:/run/php-fpm/www.sock|fcgi://localhost"`, and no
`Timeout` or `ProxyTimeout` is set anywhere under `/etc/httpd/`, so
mod_proxy_fcgi uses Apache's 60-second default. php-fpm's own
`request_terminate_timeout` is commented out and `max_execution_time` is `0`, so
**PHP keeps running after Apache has stopped listening** — the work completes,
but its response is discarded.

This is not specific to SNPversity; it is the ceiling on every request this
server can answer. It matters here because SNPversity's own time estimator
routinely answers four minutes or more for a wide region, so the *submit*
response is expected to be lost on exactly the queries that need it most.

**How the modern page handles it.** The query id is minted in the browser rather
than by the server, and it is the id that names the engine's output files. The
page therefore keeps polling a cheap status endpoint for that id alongside the
submit request, and goes to the result when the file appears — whatever happened
to the request that started it. The submit's own PHP calls
`ignore_user_abort(true)` so it finishes and caches its parse rather than being
killed halfway. The legacy page had the same 60-second ceiling and nothing to
...
`SetHandler` for the endpoints that need it, or an explicit site-wide `Timeout`.

- **Required administrator:** whoever owns `/etc/httpd/` on the instance
- **Status:** proposed, not blocking. Worked around in the page.

## AD-069 — Eleven news items in data/news.xml share an id with another item

`data/news.xml` is curator-maintained and lives only on the server
(`/var/www/claude/html/data/news.xml`, 262 `<entry>` elements). Its `<id>`
values are meant to be unique — `/whatsnew` turns each into a DOM id and a
"Link to this item" permalink — but **11 ids are reused across genuinely
different items**, and `233` is used three times:

| id | items sharing it |
|---|---|
| 233 | 2024-06-24 European Maize Meeting registration · 2024-06-18 MaizeMine v1.6 · 2024-04-17 NPGS job posting |
| 241 | 2025-06-13 Schmitz lab data · 2025-05-21 Phylostrata tracks |
| 230 | 2024-02-12 Peter Rogowsky obituary · 2023-12-08 new Data Center pages |
| 229 | 2023-10-11 McClintock Prize (Caroline Dean) · 2023-09-26 PanEffect release |
| 215 | 2021-01-08 Lauter/Auger obituaries · 2020-12-18 2021 Maize Meeting registration |
| 156 | 2017-03-02 59th Maize Meeting program · 2017-02-27 its schedule |
| 154 | 2017-02-07 EP1/F7 assemblies · 2017-01-25 lost MNL submissions |
| 152 | 2017-01-23 Buckler NAS Prize · 2017-01-19 MaGNET deadline extended |
| 134 | 2016-02-16 Maize Meeting workshops · 2016-01-04 new Editorial Board members |
| 109 | 2014-12-22 CornCyc 5.0 · 2014-12-15 Lanzanova MNL note |
| 103 | 2014-06-27 alpha site June release · 2014-05-23 Gardiner NCGA podcast |

In every case the two entries are adjacent in the file: the curator added a new
entry and did not increment the number. These are not duplicate items — the
dates and text differ.

**Already worked around, so nothing is broken today.**
`controllers/whatsnew.php` now makes the anchor ids unique in document order:
the first occurrence keeps the bare `news-<id>` so existing permalinks are
unaffected, and later ones take `-2`, `-3`. Verified: 262 ids, 262 distinct, and
all 11 previously ambiguous permalinks still resolve to the same item they did
before.

**What an administrator/curator can do:** give the 22 affected entries unique
ids in `news.xml` (the highest currently in use is 250, so 251+ are free), then
the workaround becomes a no-op. Worth doing because the workaround's synthesized
ids are positional — if an entry is later inserted between two that collide, an
already-shared `news-233-2` link would move to a different item. The bare
`news-<id>` links are stable regardless.

This repo does not edit `news.xml`: it is curator-maintained, server-only, and
not in `deploy/manifest.txt`, so a change here would be outside the deploy
system and could be overwritten.

- **Date:** 2026-09-09
- **Affected component:** `data/news.xml`, `controllers/whatsnew.php`
- **Required administrator:** MaizeGDB news curator
- **Status:** proposed
- **Validation:** every `<id>` in `data/news.xml` is distinct — `grep -o '<id>[0-9]*</id>' data/news.xml | sort | uniq -d` prints nothing.
...
- **Date:** 2026-09-11
- **Affected component:** `sequence.maizegdb.org`, `sequence2.maizegdb.org`, and the pages that call them — `search/gene/get_fasta.php` ("Download sequence for a gene model list" on the Gene Data Hub) and any other caller of `get_sequence.php`
- **Current limitation:** Both hosts answer normally to the open internet and answer

  ```
  SEQUENCE SERVICE IS DOWN.
  ```

  to requests originating from the MaizeGDB web servers. Measured 2026-09-11:

  | From | `GET https://sequence2.maizegdb.org/get_sequence.php?gene-model-set=Zm00001d.2&id=Zm00001d002005&dbtype=gene` |
  |---|---|
  | A workstation off-site | FASTA, `>Zm00001d002005` followed by sequence |
  | dev8 (`claude` vhost), over ssh | `SEQUENCE SERVICE IS DOWN.` |
  | `sequence.maizegdb.org`, off-site | FASTA |

  So this is not the service being down. It is down *for us*. The consequence is
  that `POST /search/gene/get_fasta.php` returns `SEQUENCE SERVICE IS DOWN.`
  instead of FASTA on **both** `claude.maizegdb.org` and `www.maizegdb.org` —
  the tool is broken on production, not only on the development instance.
- **Proposed change:** Find out what distinguishes those requests. The likely
  candidates are an allow-list on the sequence host that the web servers'
  addresses have fallen off, an egress rule on the web servers, or split-horizon
  DNS resolving `sequence2.maizegdb.org` to an internal address that is no longer
  listening. Whatever it is, restore it so `get_sequence.php` answers the web
  servers.
- **Expected benefit:** "Download sequence for a gene model list" starts working
  again on the live site. It also unblocks work that was scoped out because of
  this: the Gene Data Hub's BLAST hand-off cannot turn a pasted list of gene
  model IDs into FASTA while the only source of sequence refuses the call, so
  that panel takes sequence only.
- **Risk and rollback:** None to this repository. No code change is proposed.
- **Required administrator:** MaizeGDB system administrator
- **Status:** proposed
- **Validation:** From a web server, `curl -s 'https://sequence2.maizegdb.org/get_sequence.php?gene-model-set=Zm00001eb.1&id=Zm00001eb067740&dbtype=gene' | head -1` prints `>Zm00001eb067740`, and `POST /search/gene/get_fasta.php` with `gm_seq_list=Zm00001eb067740&in_type=gene&out_type=genomic` returns FASTA.
...
- **Date:** 2026-09-11
- **Affected component:** `mgdb.maize_history`, and `/maize_history` which renders it
- **Current limitation:** Rows `maize_history_id` **13** and **29** are the same
  event entered twice. Every field a reader sees is identical:

  | field | 13 | 29 |
  |---|---|---|
  | `year` | 1994 | 1994 |
  | `title` | MaizeDB | MaizeDB |
  | `description` | MaizeDB publicly announced by Ed Coe | *same* |
  | `image_caption` | The MaizeDB home page in 1994. | *same* |
  | `publication`, `pub_link`, `image_name`, `image_credit` | empty | empty |
  | `event_type` | `cooperative resource` | `cooperative_resource` |

  Only `event_type` differs, and only in its separator.
  `controllers/community/maize_history_modern.php` matches on `strpos($type,
  'coop')`, so both spellings render the same badge — the timeline simply drew
  the MaizeDB card twice, and counted it twice in the "Cooperative Resource"
  total.

  The separator is inconsistent across the whole table, not just here:
  `cooperative resource` on 8 rows and `cooperative_resource` on 6. Nothing
  reads it literally today, so that inconsistency is harmless on its own.
- **Proposed change:** `DELETE FROM mgdb.maize_history WHERE maize_history_id = 29;`
  Row 13 is the one to keep: its `event_type` uses the spelling the majority of
  the table uses. Optionally, settle the separator across all 14 cooperative
  rows at the same time.
- **Why an administrator:** the web user `mgdb` has **SELECT only** on
  `mgdb.maize_history` — verified with `has_table_privilege`, which returns
  false for INSERT, UPDATE and DELETE. The row cannot be removed from the
  application.
- **Workaround in place:** the controller now drops rows whose year, title and
  description are all identical to one already emitted, keeping the first. The
  page shows 31 cards instead of 32 and the counts agree. **Delete this
  workaround once the row is gone** — it is defensive, but it would also hide a
  second genuine duplicate rather than surfacing it.
- **Status:** proposed
...
- **Date:** 2026-09-12
- **Affected component:** `mgdb.gene_prod_metabolic_constit`, `mgdb.reference_authors`,
  `mgdb.locus_coordinates`, `mgdb.id_reference`; surfaced through
  `include/api/v1/records/*.php`
- **Current limitation:** Each record resource measures a section twice — once
  as an indexed count for `meta.counts`, once as the rows themselves — and warns
  when the two disagree, because `include/db-api.php` returns an empty result
  rather than raising, so a broken query would otherwise be indistinguishable
  from a record with no data. Carson saw one of those warnings printed on
  `/gene_center/gene/wx1`.

  Most of the disagreements were the count's fault and are fixed in the
  interface (see below). What is left is data: rows whose foreign key names a
  row that is not there. A section cannot list them, so the honest count
  excludes them, but the rows themselves are still wrong.

  | table | rows | what is missing |
  |---|---|---|
  | `mgdb.gene_prod_metabolic_constit` | 1 | gene product **13795** names metabolic constituent term **24612**, which has no row in `mgdb.term` |
  | `mgdb.reference_authors` | 116 | author ids with no row in `mgdb.person` at all — e.g. reference **1233776**, `order1` 112, author **1079715** |
  | `mgdb.locus_coordinates` | 2 | loci **86644** (`agrc362a`) and **86697** (`agrc701`) sit on maps **86784** and **86782**, which have no row in `mgdb.map` and none in `mgdb.id_num` |
  | `mgdb.id_reference` | 231 | reference ids curated in `id_num` with no row in `mgdb.reference` — 229 of them on Reference records, one Locus, one Variation |

  A further 47 `reference_authors` rows name a person who exists but is
  withheld (`id_num.curation_lvl <> 0`), and locus **12576** (`r1`) sits on map
  **1160721**, which exists but is withheld. Those are not defects — they are
  curation decisions, and the counts now respect them.
- **Proposed change:** a curator reviews each group and either supplies the
  missing record or deletes the pointing row. Nothing here should be deleted by
  a blanket statement: an author id with no person is a citation that lost its
  author, and that is worth recovering rather than dropping.
- **Why an administrator:** the web user `mgdb` has SELECT only on these tables,
  and the decisions are curatorial rather than mechanical.
- **Required administrator:** MaizeGDB curator
- **Workaround in place:** the counts now count what their section can list, so
  no page claims a row it cannot show:
  `records/gene.php` (alleles, map positions), `records/gene_product.php`
  (metabolic constituents), `records/reference.php` (authors) and
  `records/marker.php` (positions, which also gained a consistency check it
  never had). This is not defensive — a count of displayable rows is the right
  count either way — so it stays after the data is fixed.
- **Status:** proposed
- **Validation:** `php tools/tests/api_warning_sweep.php 15` reports 0 count
  mismatches over 240 records of all 17 types. The data itself: the four
  queries in the table above return 1, 116, 2 and 231 rows respectively.
...
- **Date:** 2026-09-14
- **Affected component:** `mgdb.reference`, `mgdb.ext_db_key` (`db_person`
  134209 PubMed, 2738676 DOI); surfaced by the exports on
  `/data_center/reference`
- **Current limitation:** Among the 54,900 curated references, **68 PubMed IDs
  are carried by two records each (136 records) and 4 DOIs by two each (8
  records)**. An identifier that resolves to two different MaizeGDB references
  is wrong in one of two ways, and both are present:

  | shape | PubMed IDs | reading |
  |---|---|---|
  | same year, one title between them (or one blank) | 38 | the same paper entered twice |
  | same year, two different titles | 26 | the ID is on the wrong record |
  | different years | 4 | the ID is on the wrong record |

  Twelve of the pairs include a record with **no title at all**. Worked
  examples of each shape:

  - PMID `10393983` → `223012` (1999, no title) and `223844` (1999, "High gene
    density is conserved at syntenic loci of small and large grass genomes").
    The first looks like an empty stub of the second.
  - PMID `10518597` → `952553` (1999, "Auxin-induced K+ channel expression…")
    and `9017928` (2006, "The auxin-induced K+ channel gene Zmk1 in maize…").
    Two distinct papers; at most one of them owns that PMID.
  - DOI `10.1039/c2mb05483c` → `9042175` ("Multiple-strategy analyses of ZmWRKY
    subgroups…") and `9042180` ("Molecular phylogenetic and expression analysis
    of the complete…"). Same year, different papers.
  - DOI `10.1111/j.1744-7909.2010.00906.x` → `2804563` and `2804566`, identical
    titles: the same paper twice.

  Separately, and cheap to fix at the same time: **two references carry the
  literal string `none` in `mgdb.reference.doi`**, and the DOI store in
  `ext_db_key` holds `dup`, `doi`, `123`, `1`, values written as
  `doi: 10.x/y`, `DOI 10.x/y` and `https://doi.org/10.x/y`, one with a leading
  slash, and a handful mangled with invisible characters.
- **Proposed change:** a curator reviews the 68 + 4 groups and, per group,
  either merges the duplicate records or removes the identifier from whichever
  record does not own it. The junk DOI values should be corrected to a real DOI
  or emptied.
- **Why an administrator:** the web user `mgdb` has **SELECT only** on both
  tables — `has_table_privilege` returns false for UPDATE and DELETE on
  `mgdb.reference` and on `mgdb.ext_db_key`. Neither the merge nor the cleanup
  can be done from the application.
- **Required administrator:** MaizeGDB curator
- **Workaround in place:** the DOI and PubMed exports emit a **de-duplicated**
  list, so a shared identifier appears once, and `referenceDoiSql()` in
  `search/reference/reference_search_lib.php` extracts a DOI by pattern so the
  junk values are not exported as identifiers. The page states the shortfall
  before a reader downloads: "8,479 of the 54,900 matches carry one, and 4
  share one with another reference." **That explains the arithmetic; it does
  not fix the records** — a search for either paper still returns both, and the
  duplicate pairs are still counted twice in every figure on the page.
- **Status:** proposed
...
- **Date:** 2026-09-14
- **Affected component:** `mgdb.reference`; visible in the four figures on
  `/data_center/reference` and in any count of papers per year
- **Current limitation:** Journal-article curation is not steady over time.
  Excluding Maize Genetics Conference abstracts and MNL articles, which are
  separate collections with their own figures, the count per year runs:

  | 1995 | 2000 | 2005 | 2010 | 2015 | 2020 | 2025 |
  |---|---|---|---|---|---|---|
  | 1,417 | 803 | 409 | **144** | 263 | 580 | 1,211 |

  Carson raised the 2010 figure as a suspected charting bug. It is not: 144 is
  what the table holds. The decline is steady from the mid-1990s, bottoms in
  2010, and recovers to roughly the 1990s rate by 2025 — a real gap in what was
  curated, about fifteen years wide, in the period most users search.

  Two collections also simply stop:

  - **Maize Genetics Conference abstracts** are loaded through 2016 (414 that
    year), then nothing for 2017, 2018 or 2019, 273 for 2020, and **nothing for
    2021 through 2026**.
  - **MNL articles** run to 2014 (5 that year), with a single stray record in
    2017 and nothing since.
- **Proposed change:** a curator confirms whether the 2005–2019 shortfall and
  the two stopped collections are a known backlog or an unnoticed break in a
  loading process, and schedules the missing literature for loading. If some of
  it is deliberate — a change in scope rather than a gap — that decision should
  be recorded so the figures can say so.
- **Why an administrator:** loading literature is curation. The web user has
  SELECT only on `mgdb.reference`, and in any case the question of what belongs
  in the collection is not one the application can answer.
- **Required administrator:** MaizeGDB curator
- **Workaround in place:** none. The figures render the table faithfully, which
  is the problem: a reader comparing 2010 with 1995 sees a statement about the
  maize literature where the data supports only a statement about MaizeGDB's
...
- **Date:** 2026-09-14
- **Affected component:** `chado.transcript` (`translation_name`); every page and
  API that names the protein an isoform encodes — the gene record's Sequences
  and Transcripts tables, `include/api/v1/records/gene.php`, and any legacy
  reader of that column
- **Current limitation:** The column is populated **only for the transcript
  marked canonical**. For B73 v5 (`Zm00001eb.1`) it names a protein for 39,756
  of the 72,539 protein-coding transcripts, so **32,783 have none**; every NAM
  founder annotation is short by about the same 32,000, and B73 v4
  (`Zm00001d.2`) by 89,302 of 131,318. B73 v1 (`4a`) and v2 (`5b`) have no
  translation names at all.

  The proteins themselves exist. `sequence2.maizegdb.org` serves
  `Zm00001eb168550_P002`, `_P003` and `_P004` as readily as `_P001`, and the
  published GFF3 names a protein for all 72,539 v5 protein-coding transcripts.
  The gap is in this column, not in the annotation.
- **Requested:** load `translation_name` for every protein-coding transcript
  from the annotation GFF3, not only for the canonical one. The value is the
  transcript name with the `T` of the numeric suffix turned into a `P`
  (`_T002` → `_P002`; v3's `_FGT001` → `_FGP001`), which holds for all
  1.3 M rows the column already carries.
- **Worked around, 2026-09-14:** `gene_api_transcript_protein()` in
  `include/api/v1/records/gene.php` fills the name in — from the gene-models
...
- **Date:** 2026-09-15
- **Affected component:** `tools/sequence/get_sequence.php`;
  `sequence2.maizegdb.org`; `fasta.maizegdb.org`;
  `download.maizegdb.org/B73_RefGen_v1/`, `/B73_RefGen_v2/`; the NCBI BLAST+
  install on dev8; SELinux labels under `/home/cache`
- **Current limitation:** The sequence server was hardened and made local-first
  ...
  **2. B73 v1 and v2 have no FASTA index published.** Every `.fa.gz` under
  ...
  `.gzi` beside it, so `fasta.maizegdb.org` cannot open them —
  `"Unable to open file"` — and every v1 and v2 sequence, protein and position
  request has always come back empty. v3 and the modern assemblies have both
  index files and work.
  ...
  **3. Two small things on dev8.** Neither is urgent; both remove a workaround.
  - `blastdbcmd` is not installed. `/usr/bin` has `blastn`, `blastp` and
    `makeblastdb` but not `blastdbcmd`; the copies in each vhost's
    `popcorn/utilities/` are mode 644 and link against a `libnsl.so.1` this OS
    no longer ships; the working 2.16.0+ build is under `/home/ethy/installs/`,
    which is mode 700. With it installed, the BLAST databases already on the
    NFS mount (`/home/Data/Blast/REFGEN_V5/gene_models/…`) could serve
    sequences by identifier with no local mirror at all.
    - **Requested:** copy `/home/ethy/installs/ncbi-blast-2.16.0+/bin/blastdbcmd`
      to `/usr/bin` beside the other BLAST binaries.
  - `/home/cache` is labelled `user_home_dir_t`, so the web user cannot create
    a subdirectory of it — `/home/cache/dashboard` and `/home/cache/search`
    were each given `httpd_sys_rw_content_t` by hand. The sequence cache
    therefore falls back to the php-fpm private tmp, which is cleared whenever
...
- **Date:** 2026-09-25
- **Affected component:** `/foldseek`; `search/foldseek/foldseek_lib.php`; the
  application at `foldseek.maizegdb.org`
- **What it is:** `/foldseek` shows the precomputed Foldseek analysis — the
  39,299 maize AlphaFold DB version 3 models, each searched against eight
  proteomes. The hit tables, alignments and C&alpha; coordinates exist only
  as the HTML that application serves: a `render([...])` call inside a 7.6 MB
  page per protein. Not in the MaizeGDB database, not on dev8, not in any
  export. The redesigned page fetches that page once per protein, parses it
  and caches the result for 30 days under `<search_cache_path>/foldseek`. The
  same arrangement as `/fatcat` (AD-029), for the same reason.
- **What the administrator is being asked for:**
  1. **The source data behind the application** — the Foldseek result files,
     or whatever the page reads them from. With them, `/foldseek` can be built
     offline into a local index the way `/data_center/protein_structure` and
     `/data_center/alphafill` are, and stops depending on another host being
     up and on its markup not changing.
  2. **A fix, or a notice, for the upstream viewer's TM-score.** For 13 of
     bz1's 165 matches the page scores a truncated structure. It rebuilds each
     C&alpha; trace with PULCHRA; where PULCHRA leaves a stretched peptide
     bond (1.98 Å C–N on the sorghum match `A0A1Z5R994`) NGL splits the model
     into chains A and B, and TM-align, which reads only the first chain,
     scores 390 of the 498 residues. The page shows **0.72573**; TM-align on
     the whole structure gives **0.77119**. `/foldseek` computes the second;
     the old page, still live, shows the first to anyone who reaches it
     directly.
- **Worked around 2026-09-25:** `js/mgdb-tmalign.js` ports TM-align's `-I`
  scoring and runs it on the search's own coordinates, which need no rebuild
  (PULCHRA moves no C&alpha; atom). Verified against the upstream display on
  the 152 untruncated matches and against its own TM-align binary on the
  other 13.
- **Also recorded:** the upstream answers an identifier it does not know with
  an **empty HTTP 500**, which the adapter reads as "not in the analysis"; it
  matches identifiers case-sensitively (`bz1` resolves, `BZ1` does not); and
  its AlphaFold model links point at `model_v3`, retired by EMBL-EBI, where
  `/foldseek` links version 6.
- **Expected benefit:** `/foldseek` independent of another host, and the
  right TM-score on the old page too.
- **Required administrator:** MaizeGDB application maintainer (the
  `foldseek.maizegdb.org` host)
- **Status:** proposed
- **Validation:** with the source data local, `php tools/tests/foldseek_adapter_check.php`
  still passes against the new reader; on the upstream page, row 60 of
  `?uniprot=Zm00001eb374230` shows TM-Score 0.77119.
...
- **Date:** 2026-09-25
- **Affected component:** the header search's suggestions
  (`data/suggest/header.sqlite`), every Data Hub search box's suggestions
  (`data/suggest/<scope>.sqlite`), and the data-centre dashboard cache (AD-023);
  `tools/nightly_rebuild.php`
- **What it is:** Three things are built from the database and do not follow
  it on their own: the hub suggestion index (`tools/suggest_index.php`, about
  5 minutes, about 1 GB), the header search index (`tools/header_index.php`,
  about 2.5 minutes, about 600 MB), which must return exactly the rows the
  header's own SQL returns, and the dashboard cache
  (`tools/dashboard_cache.php --purge --warm`). After a reload they describe the
  old database: new records are not suggested, a renamed or removed locus or
  gene model can still be suggested and leads to a not-found page, and the
  header index's copy of Postgres's physical row order no longer matches, so
  its suggestions drift from the live SQL's. `tools/nightly_rebuild.php`
  rebuilds all three whenever the database has changed, then holds the header
  index to parity with the live SQL; if that fails it sets the header index
  aside, and the header answers live -- slower, exact, never stale -- until a
  good build replaces it.
- **Installed on the development instance, 2026-09-25,** in the `carson`
  account's crontab on dev8:

      30 2 * * * cd /var/www/claude/html && /usr/bin/php tools/nightly_rebuild.php

- **What the administrator is being asked for, on each instance that serves
  the redesign (production first):**
  1. Deploy as usual. `deploy/manifest.txt` carries the job, the three
     builders, the parity test and its query list; the indexes themselves are
     never deployed, each host builds its own.
  2. As the account that owns the web root -- the builders write
     `data/suggest/`, the web server only reads it -- run the first build by
     hand and read its line in the log:

         cd <webroot> && php tools/nightly_rebuild.php --force
         tail -1 data/suggest/nightly.log

     It should read `rebuilt -- suggest ok; header ok; dashboards ok; parity
     633 compared: 633 identical, 0 differ`.
  3. Install the cron entry for that account, at an hour the monthly reload
     will have finished by:

         30 2 * * * cd <webroot> && /usr/bin/php tools/nightly_rebuild.php

     Where the reload is a script, also end it with
     `cd <webroot> && php tools/nightly_rebuild.php --force`: the rebuild then
     follows the reload exactly, and the nightly run finds nothing to do. A run
     during a reload is not harmful -- it records the database as it was, so the
     next night sees the change and builds again -- but it wastes a night.
  4. Prerequisites the job cannot supply itself:
     - **Disk:** 3 GB free on the web root's filesystem. Each index is written
       beside the live one before the swap, and the previous copy is kept for
       rollback; the job refuses to start with less.
     - **SELinux:** `data/suggest/` keeps the web server's file context. The
       builders write in place for that reason; do not build elsewhere and move
       the files in.
     - **Collation:** the web host's glibc must order text as the database
       host's does (glibc 2.34 on both at dev8). The header index compares names
       with PHP's `strcoll()` in en_US.UTF-8; `tools/header_index.php` checks
       every row against the database's order and stops on the first
       disagreement, and the job then sets the header index aside -- the header
       keeps working, live.
     - **Dashboard cache:** its directory and SELinux context (AD-023), where
       `dashboard_cache=true`.
  5. On the curation instance, whose database takes writes all day, either
     accept a rebuild every night (about 10 minutes of reads) or leave the
     header index off there (`AC_HEADER_INDEX` false in
     `controllers/search_engine/autocomplete.php`) and do not install the job.
- **What counts as a change:** each table's file number (`pg_class.relfilenode`)
  and write counters (`pg_stat_all_tables`), compared with the last good build:
  a reload, a refreshed materialized view or a curation edit moves them, a
  vacuum or an analyze does not. `perm_tables.record_access` (record views) and
  `mgdb.pc_job_ctl` (BLAST jobs) change in ordinary use and are ignored. A run
  that rebuilds names the tables that caused it in `nightly.log`; a production
  table that triggers a rebuild every night can be added to `$NB_IGNORE` in the
  script, or passed as `--ignore=schema.table`.
- **Expected benefit:** suggestions that match the database within a day of
  any change, with nobody having to remember a rebuild; a failure falls back to
  exact live answers rather than stale ones.
- **Risk and rollback:** Low. The job reads the database for about 10 minutes,
  only on nights after a change. Removing the cron line stops it, and the
  indexes keep serving as last built. `AC_HEADER_INDEX` false switches the
  header to live answers without touching the index.
- **Required administrator:** System administrator (a cron entry on each web
  host)
- **Status:** proposed for production. Implemented on the development instance
  2026-09-25.
- **Validation:** `php tools/nightly_rebuild.php --status` reports the last good
  build and anything changed since; `data/suggest/nightly.log` gains a line
  every night, `unchanged` on most; the header's responses carry
  `X-Suggest-Engine: index`.
...
- **Date:** 2026-09-25
- **Affected component:** `/fusarium` and its pages; `tools/fusarium_index.py`;
  the application at `fusarium.maizegdb.org`
- **What it is:** `/fusarium` recreates the Fusarium Protein Toolkit on the
  design system (README, "Fusarium Protein Toolkit"). Its models come from the
  toolkit's host through this site (AD-084), and its Foldseek results through
  the `/foldseek` adapter with `set=fusarium`. Four things there need the
  host's administrator:
  1. **The ESMFold directories of F. fujikuroi, F. oxysporum, F. proliferatum
     and F. solani answer HTTP 403 for every file** —
     `protein_structure/structures/esm_{fujikuroi,oxysporum,proliferatum,solani}/`,
     25 of 25 sampled each — while they list their 14,336, 30,405, 15,646 and
     15,301 models and the other eight directories serve every file. Almost
     certainly file modes or an access rule on that host. Until it is fixed
     those 75,688 models are only in the Box archive, and the Structures page
     says so instead of offering them.
  2. **Where the toolkit lives.** The new pages answer the old paths
     (`index.php`, `effector.php`, `help.php`,
     `protein_structure/index.php?uniprot=`) with 301s, so the old hostname
     could be pointed at them without breaking a link — or the old site can
     stay as it is. A decision, not a change this repository can make.
  3. **CORS -- no longer needed by the viewers.** The host sends
     `Access-Control-Allow-Origin` for MaizeGDB origins, but since 2026-09-25
     the pages read models through this site instead (AD-084), so a new host
     serving `/fusarium` needs nothing added here.
  4. **Analytics.** The old pages load Google Analytics `G-DMZ75NENB5`; the
     new ones load no tracker. Whether production keeps one is the
     toolkit's call.
- **Also for the toolkit's maintainers** (the new pages work around each; the
  old site still shows them): the help page's title reads "Fusarium Proten
  Toolkit"; its genome table gives F. venenatum 113,945 proteins (UniProt:
  13,945); the Foldseek text says "nine proteomes" and lists ten (F.
  verticillioides is not a target); every F. fujikuroi, F. oxysporum,
  F. proliferatum and F. solani row of the effector table links Foldseek and
  PanEffect pages that are empty, and 255 F. oxysporum rows link AlphaFold DB
  pages for deleted entries; `record_data/protein_structure_data.php` resolves
  two species and labels anything else F. graminearum; ten F. graminearum
  effectors carry no EffectorP probability.
- **Expected benefit:** ESMFold models and the AlphaFold-versus-ESMFold
  comparison for all six species, and a settled home for the toolkit.
- **Required administrator:** MaizeGDB application maintainer (the
  `fusarium.maizegdb.org` host)
- **Status:** proposed
- **Validation:** `curl -I` on any file in the four `esm_` directories returns
  200; `python3 tools/fusarium_index.py ...` then logs no "(NOT SERVED)", and
  `/fusarium/structures?id=FFUJ_00038` enables its ESMFold and Compare tabs.
...
- **Date:** 2026-09-25
- **Affected component:** `lib/Bauplan.php` (`bundle()`); the gene record,
  `controllers/gene_center/gene_record_v5.php`, the one page that opts in so
  far; the directory `<webroot>/temp/bundles/`
- **What it is:** The gene record now serves its fourteen stylesheets and eleven
  scripts as four files, which Bauplan builds on the first request that needs
  them and writes into `temp/bundles/` (README, "Bundling a page's stylesheets
  and scripts"). On the development instance that needed no administrator:
  `temp/` is `john:mgdbadmin` mode 775, Apache is in `mgdbadmin`, and a
  standing SELinux rule, `/var/www/claude/html/temp(/.*)?` →
  `httpd_sys_rw_content_t`, labels anything created under it. The first request
  created the directory as `apache:apache`. A host whose `temp/` has other
  ownership or another label cannot, and the page then serves its 25 original
  tags -- correct, only unbundled -- with `<!-- bundle ... not built: ... -->`
  in its `<head>`.
- **What the administrator is being asked for, on each production host:**
  confirm that Apache can create `temp/bundles/`, or create it once, owned by
  the web server user and labelled `httpd_sys_rw_content_t` -- with a
  `semanage fcontext` rule for `temp(/.*)?` like the development instance's,
  so the label survives `restorecon`. The directory's `.htaccess`, which
  Bauplan writes, sends `Cache-Control: public, max-age=31536000, immutable`;
  that needs `mod_headers` and `AllowOverride FileInfo`, which the site's own
  routing `.htaccess` already requires.
- **Expected benefit:** the gene record's first visit is nearly the same either
  way (700 ms bundled, 704 ms unbundled, 10 Mbps cold cache); the difference is
  on the repeat visit. Unbundled, every one of the 25 files is stale at the edge
  after 30 minutes and is revalidated before the page can paint; bundled, four
  files are kept by the browser for a year and nothing is asked. The Plotly
  change, which needs no administrator, is what took the first paint from
  1,848 ms to 700 ms.
- **Risk and rollback:** Low. Nothing but Bauplan reads or writes the
  directory, a bundle is only ever a copy of files already public under `/css`
  and `/js`, and superseded bundles are removed an hour after they are
  replaced. Rollback: delete the `bundle()` line from the controller; the
  directory can then be deleted.
- **Required administrator:** System administrator (each production web host)
- **Status:** proposed for production. Not needed on the development
  instance, where it works as is.
- **Validation:** `curl -s https://<host>/gene_center/gene/Zm00001eb067740 |
  grep -c "temp/bundles/"` returns 4, the same page contains no `not built`
  comment, and `curl -sI` on one of the four URLs shows the one-year
  `Cache-Control`.
...
- **Date:** 2026-09-25
- **Affected component:** `/fusarium/structures` and `/fusarium/foldseek`
  (their 3D viewers); `include/fusarium_lib.php` (`fptModelLink`,
  `fptModelText`); `search/fusarium/fusarium_api.php?action=model`; the
  `fusarium.maizegdb.org` zone settings in Cloudflare
- **What happened:** reported 2026-09-25 -- the structure viewers drew on
  some computers and not on a phone or a laptop. Measured the same day:
  requests from the Iowa State network (AS2698) reach `fusarium.maizegdb.org`
  directly; from off campus Cloudflare answers with its "Performing security
  verification" page. A person opening that host can pass the check, but the
  pages fetched each model file in the background, cross-origin and without
  cookies, which can neither pass it nor carry a pass -- so the viewer received
  the check's HTML instead of a PDB file. `claude.maizegdb.org`,
  `www.maizegdb.org` and `images.maizegdb.org` are not checked the same way,
  which is why the maize structure pages were unaffected.
- **Worked around 2026-09-25:** the pages now read models from their own
  origin -- `search/fusarium/fusarium_api.php?action=model&model=alphafold&term=<accession>`
  -- and the web server, which is on campus, fetches each file from the
  toolkit host once and keeps it for 90 days in `<search_cache_path>/fusarium`
  (or `fusarium_cache_path` in `conf/mgdb.conf`). Only models the local index
  lists can be asked for, so it fetches no URL a caller chooses. A side
  benefit: "Download PDB" now downloads with the file's name, which a
  cross-origin link cannot do.
- **What the administrator is being asked for:**
  1. **Decide whether the check is meant to cover the model files.** If not,
     exempt `fusarium.maizegdb.org/protein_structure/structures/` (static PDB
     files) from the bot check, or lower that host's security level to match
     the rest of `maizegdb.org`. The workaround can stay either way.
  2. **On production, keep the web server where the check does not reach it,**
     or exempt its address. The workaround depends on the server's own
     request passing: from dev8 it does (campus). A production web host off the
     Iowa State network would be checked too, and every model would fail
     again -- this time server-side, logged as a 502 from `action=model`.
  3. **The cache directory's label** on each web host, as for AD-023:
     `mkdir -p /home/cache/fusarium && chmod 777 /home/cache/fusarium` and
     `semanage fcontext -a -t httpd_sys_rw_content_t '/home/cache/fusarium(/.*)?'`
     then `restorecon -Rv /home/cache/fusarium`. Without it everything works
     and each model is fetched afresh.
- **Expected benefit:** the viewers work for every visitor, not only on campus.
- **Risk and rollback:** Low. Reverting `fptLinks()` and `fsModelUrl()` to
  `fptModelUrl()` restores direct links (and the off-campus failure).
- **Required administrator:** Network administrator (the Cloudflare
  configuration for `fusarium.maizegdb.org`), with the system administrator
  for the cache directory
- **Status:** proposed. The workaround is live on the development instance.
- **Validation:** from off campus, `curl -sI https://fusarium.maizegdb.org/protein_structure/structures/verticillioides/AF-A0A139YB70-F1-model_v4.pdb`
  returns 200 rather than 403 with a challenge; and
  `/fusarium/structures?id=FVEG_13850` draws its model on a phone on cellular
  data.

## AD-085 — Annotation files on download.maizegdb.org carry format defects that every reader has to work around

- **Date:** 2026-09-25
- **Affected component:** the annotation GFF3 and protein FASTA files published
  under `download.maizegdb.org/<genome>/`; `tools/gene_models_index.py` and
  `tools/gene_models_all.py`, which build the Data API's gene-model releases
  from them (README, "Every genome")
- **What happened:** building a release for every genome on the host meant
  reading 134 annotations, and a profile of every file found them written in a
  dozen conventions. Most are legitimate variation, which the builder now reads
  and counts in each release's manifest. The ones below are defects: a GFF3
  validator rejects them, or the files contradict each other. Each was
  measured against the published file on 2026-09-25.
  1. **Mo17 CAU-1.0** (`Zm00014a.1.gff3.gz`) spells every UTR type
     `five_P00rime_UTR` / `three_P00rime_UTR` (43,091 and 44,984 rows) -- a
     find-and-replace that turned `p` into `P00`. Any reader looking for
     `five_prime_UTR` finds no UTRs in this assembly.
  2. **LH244 CAU-1.0** (`Zm00116aa.1.gff3.gz`) writes 38,431 of its mRNA rows
     with the source and type joined by an underscore (`EVM_mRNA`), so those
     rows have eight columns instead of nine and every GFF3 parser drops them,
     orphaning their exons and CDS.
  3. **The twelve CAAS_FIL assemblies** (A632, Chang-7_2, Dan340, Huangzaosi,
     Jing724, Jing92, Oh43, PH207, S37, Xu178, Ye478, Zheng58) give no
     `Parent` on any mRNA row, so nothing links a transcript to its gene. The
     builder infers it from the ID (`..._T001` belongs to the gene it extends
     and lies inside).
  4. **CML247 PANZEA-1.0** numbers its protein FASTA for other transcripts
     than its GFF3 does: `Zm00006a036466_T001`'s CDS is 525 nt (a 175-residue
     protein), but `Zm00006a036466_P001` is 234 residues. 27,721 of 46,313
     transcripts cannot be matched to a protein this way, and are published
     without one rather than with the wrong one. Its GFF3 also names 23 exons'
     transcript by its maker Alias instead of its ID.
  5. **Nine PanAnd annotations** (Ag, Cc, Cr, Cs, Zd-Gigi, Zd-Momo, Zh, Zn,
     Zv-TIL01) list every CDS row twice, exactly -- 1,365,175 duplicate rows in
     Ag alone. A reader that does not deduplicate doubles every CDS length.
  6. **Mo17 YAN-1.0 and Zx YAN-1.0** use serial numbers as IDs (`ID=25848`;
     the gene name is only in `Name=`) and publish no protein FASTA, so their
     proteins have no names or lengths at all.
  7. **Genome FASTA indexes that disagree with the annotation's sequence
     names** -- `1` where the GFF3 says `chr1` (Mo17 YAN, Zx YAN, F2 AMZ, and
     one sequence of F252 AMZ), `Chr1` for `chr1` (B73 RefGen_v3), `chr01`
     for `chr1` (PH207 NS-UIUC_UMN) -- and **sequences no file gives a length
     for**: 5,899 in nine releases, most in Cs (3,207), Cc (1,011), P8860
     TeoGa (922) and CIMBL55 (527), which publish no genome index or one that
     omits them. Their lengths can be read only from the GFF3's own sequence
     rows, which most annotations omit; the releases record the last gene's
     end instead, marked as such.
  8. **Cc PanAnd** names its protein FASTA by transcript (`>..._T001`) while
     its GFF3 names the proteins `..._P001`; the B104 draft names its proteins
     by maker's mRNA names (`maker-scaffold1.188-...-mRNA-1`) rather than the
     release's IDs.
  9. **CIMBL55 CAU-1.0**: three mRNAs name themselves as their own Parent.
- **Worked around 2026-09-25:** every item is read as the file evidently means
  it and counted in the release manifest (`normalized`, `duplicate_rows`,
  `protein_ids_from`, `protein_lengths_keyed_by`, `sequences[].length_from`),
  so each release says how it was read. Nothing is guessed: a protein name the
  files do not state is used only when the protein file has it at the CDS's
  length.
- **What the administrator is being asked for:** correct the files at the
  source when they are next republished -- items 1, 2, 3, 5 and 9 are
  mechanical fixes to the GFF3; item 4 needs the protein FASTA regenerated
  from the GFF3 it accompanies; items 6 to 8 need the missing or differently
  named companion files published. After a republish,
  `python3 tools/gene_models_all.py --dest data/gene_models` rebuilds only the
  releases whose GFF3 changed, and each rebuilt manifest shows the workaround
  counts drop to zero.
- **Expected benefit:** every reader of these files -- not only this site --
  gets correct UTRs, transcripts, proteins and lengths without its own
  workarounds.
- **Risk and rollback:** None to the site; the workarounds stay valid on
  corrected files.
- **Required administrator:** MaizeGDB curator (whoever publishes
  download.maizegdb.org)
- **Status:** proposed. The workarounds are live on the development instance.
- **Validation:** for a republished genome, `/api/v1/data/gene-models/<genome>`
  reports `normalized` empty, `duplicate_rows` empty and
  `protein_ids_from` without `"none"`.
