...
83 entries have been raised. **71 are open** and **12 are closed** — applied, implemented, or recorded as needing no administrator at all. Regenerated by `tools/admin_index.py` on 2026-09-25; the entry is always the authority, this table is only a way in.
...
- **What it is:** `/fusarium` recreates the Fusarium Protein Toolkit on the
  design system (README, "Fusarium Protein Toolkit"). Its models are read in the
  browser straight from the toolkit's host, and its Foldseek results through
  ...
  3. **CORS.** The host sends `Access-Control-Allow-Origin` for
     `claude.maizegdb.org` and `www.maizegdb.org` only. Any other host that
     serves `/fusarium` needs adding, or the viewers cannot read a model.
