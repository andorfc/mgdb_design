...
function show_top($tmpl, $id, $DBConn) { 
  ...
  $query_record = "SELECT * FROM reference WHERE id = $id";
...
function showAnnotations($tmpl, $id, $DBConn) {
  ...
  $sql = "SELECT name FROM reference WHERE id=$id";
  $sth = make_query($DBConn, $sql);
...
function show_offsite_resources($tmpl, $id, $DBConn)  {
  $query_keys = "
    ...
    WHERE k.id= $id AND idn.curation_lvl = 0";
  ...
  if (sizeof($stock_id_array) > 0) {
    if (sizeof($stock_id_array) > 30) {
      $query_stock = "
        ...
        WHERE c.reference = $id AND c.id = b.id
...
//      $thumbnail_url = preg_replace("/(.*)\/(.*\.\w+)/", "$1/downsized/$2", $term_full_url);
    ...
    if ($arrRelatedTraits['type_name'] == 'Trait') {
      $trait_results[] = array(
        ...
        'trait_caption'  => $arrRelatedTraits['caption']);
    }
    else {
      // These are terms
      if (isset($arrRelatedTraits['contents']) && $arrRelatedTraits['contents'] != '') {
        $term_content_results[] = array(
          ...
          'term_caption'  => $arrRelatedTraits['caption']);
      }
      else {
        $term_results[] = array(
          'term_id'       => $arrRelatedTraits['id'],
...
function read_abstract($DBConn, $id) {
  $query_abs = "
    ...
    WHERE ra.id= $id AND ra.id = idn.id AND idn.curation_lvl = 0";
...
function read_authors($DBConn, $id) {
  $query_authors = "
    ...
    WHERE a.id = $id and b.curation_lvl = 0 ORDER BY ORDER1";
...
function read_genome_assemblies($DBConn, $id) {
  $sql = "
    ...
    WHERE t.name = 'MaizeGDB_reference' and pp.value='$id'";
  $sth = make_query($DBConn, $sql);
  return get_all_rows($sth);
...
function lookupurlprefix($DBConn, $arg1) {
  $query = "SELECT url_prefix FROM person_url_prefix WHERE id=$arg1";
  $stmt  = make_query($DBConn, $query);
...
function lookupextdb($DBConn, $arg1) {
  $query = "SELECT name FROM person WHERE id=$arg1";
  $stmt  = make_query($DBConn, $query);
  if ($row=retrieve_row($stmt)) {
    return $row['name'];
  }
...
function build_pubmed_url($DBConn, $id, $title) {
  ...
  $query = "SELECT id, key FROM ext_db_key WHERE id = $id AND db_person = $pubmed_id"; 
  $stmt = make_query($DBConn, $query);
