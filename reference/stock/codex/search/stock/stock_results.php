 ...
 */
  ...
  $raw_term = urldecode(getCGIParam('term', 'GP', getCGIParam('stock_term', 'S', '')));
  $url_term = urlencode($raw_term);
  $display_term = htmlspecialchars($raw_term, ENT_QUOTES, 'UTF-8');
  $term_json = json_encode($raw_term, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT);
  ...
  $template->get('raw_term')->replace($display_term);
  $template->get('term')->replace($display_term);
  $template->get('term_json')->replace($term_json);
...
//  $arrStock = getCGIParam("stock_".$term."_".$search_limit."-".$case, "S", false);
  ...
  if (!isset($arrStock)) {
    ...
    $totalAvailable = ($arrStock && isset($arrStock[0]['total_count'])) ? (int)$arrStock[0]['total_count'] : count($arrStock);
  ...
  if (!isset($totalAvailable)) $totalAvailable = $stockCount;
  ...
  if ($stockCount == 1 && !$alldata && $main != "initial_true") {
    ...
    if ($stockCount == 0) {
      ...
      if ($search_limit > 0 && $stockCount >= $search_limit) {
        ...
        $template->get('countAll')->replace(number_format($totalAvailable));
...
function handleSubsequentPage($DBConn, $pagenum, $pagesize, $raw_term) {
  $bauplan = new Bauplan('Results page');
    ...
    $arrStock = @unserialize(urldecode($rows), array('allowed_classes' => false));
    if (!is_array($arrStock)
        || !isset($arrStock['exactMatches'], $arrStock['startMatches'], $arrStock['otherMatches'])) {
      $arrStock = array('exactMatches' => array(), 'startMatches' => array(), 'otherMatches' => array());
    }
  ...
  if ($pagenum < $pagecount) {
    ...
    $tmpl->get('term')->replace(htmlspecialchars($raw_term, ENT_QUOTES, 'UTF-8'));
    $tmpl->get('raw_term_json')->replace(json_encode($raw_term, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT));
  }
...
function organize_results($arrStock, $term) {
  $unavailable = "<br><span class='stock-status stock-status-unavailable'>Unavailable</span>";
  $discontinued = "<br><span class='stock-status stock-status-discontinued'>Discontinued</span>";
